
<div align="center">

<img src="https://github.com/Deependrakashya/MY-Docs-Releases/blob/main/assets/promotional_image.png?raw=true" alt="My Docs Logo" width="120" height="120"/>

# <img src="https://github.com/Deependrakashya/MY-Docs-Releases/blob/main/assets/logo.png?raw=true" alt="My Docs Logo" width="40" height="40"/> My Docs

### Your All-in-One Offline Document Viewer & Editor

**View, Edit, Convert, and Manage PDFs, Word, Excel & more — completely offline.**

[![Platform](https://img.shields.io/badge/Platform-Windows%20%7C%20macOS%20%7C%20Linux-blue?style=for-the-badge)](https://github.com/Deependrakashya/MY-Docs-Releases/releases)
[![Version](https://img.shields.io/badge/Version-1.1.0-green?style=for-the-badge)](https://github.com/Deependrakashya/MY-Docs-Releases/releases)
[![License](https://img.shields.io/badge/License-Free-orange?style=for-the-badge)](https://github.com/Deependrakashya/MY-Docs-Releases/releases)
[![Offline](https://img.shields.io/badge/Works-100%25%20Offline-purple?style=for-the-badge)](#)

</div>

---

## 🚀 Download Now

> Click the button for your operating system to download instantly. No account or sign-up required.

<div align="center">

| 🪟 Windows | 🍎 macOS | 🐧 Linux |
|:---:|:---:|:---:|
| [![Download for Windows](https://img.shields.io/badge/Download-Windows-0078D6?style=for-the-badge&logo=windows)](https://github.com/Deependrakashya/MY-Docs-Releases/releases/download/Release/mydocs_windows.exe) | [![Download for macOS](https://img.shields.io/badge/Download-macOS-000000?style=for-the-badge&logo=apple)](https://github.com/Deependrakashya/MY-Docs-Releases/releases/download/Release/mydocs_mac_os.zip) | [![Download for Linux](https://img.shields.io/badge/Download-Linux-E95420?style=for-the-badge&logo=ubuntu)](https://github.com/Deependrakashya/MY-Docs-Releases/releases/download/Release/mydocs_linux.zip) |
| `.exe` installer | `.zip` archive | `.zip` archive |

</div>

---

## 🖥️ How to Install

### 🪟 Windows

1. Click the **Download for Windows** button above.
2. A file named **`mydocs_windows.exe`** will download.
3. Double-click the `.exe` file to run the installer.
4. If Windows shows a warning ("Unknown publisher"), click **More Info → Run Anyway**.
5. Follow the on-screen steps and click **Install**.
6. Once done, find **My Docs** in your Start Menu and launch it! 🎉

---

### 🍎 macOS

1. Click the **Download for macOS** button above.
2. A file named **`mydocs_mac_os.zip`** will download (check your `Downloads` folder).
3. **Double-click** the `.zip` file — it will automatically extract into a folder.
4. Open the extracted folder and drag **My Docs.app** into your **Applications** folder.
5. The first time you open it, macOS may say it "can't be opened because it's from an unidentified developer."

   **To fix this:**
   - Open **System Settings** → **Privacy & Security**.
   - Scroll down and click **"Open Anyway"** next to the My Docs message.
   - Click **Open** on the confirmation popup.

6. My Docs will now open normally every time! 🎉

> **💡 Tip for macOS:** When you open a PDF or document file in Finder, you can right-click it → **Open With → My Docs** to view it directly.

---

### 🐧 Linux

1. Click the **Download for Linux** button above.
2. A file named **`mydocs_linux.zip`** will download.
3. Right-click the `.zip` file → **Extract Here** (or use `unzip mydocs_linux.zip` in Terminal).
4. Open the extracted folder.
5. Find the file named **`mydocs`** (no extension) and double-click it to run.

   If it doesn't open, you may need to make it executable first:
   - Right-click the `mydocs` file → **Properties** → **Permissions** tab → Check **"Allow executing file as program"**.
   - Or in Terminal: `chmod +x mydocs && ./mydocs`

6. My Docs will launch! 🎉

---

## ✨ What Can My Docs Do?

My Docs is a **free, offline document manager** packed with powerful features:

### 📄 View Documents
| Feature | What It Means |
|---|---|
| **PDF Viewer** | Open and read any PDF file smoothly |
| **Word Viewer** | Read `.doc` and `.docx` files without Microsoft Word |
| **Excel Viewer** | View `.xls` and `.xlsx` spreadsheets |
| **CSV Viewer** | Open data files (`.csv`) as clean, readable tables |

### ✏️ Edit & Create
| Feature | What It Means |
|---|---|
| **Excel Editor** | Tap any cell in a spreadsheet to edit its value |
| **Create PDF from Images** | Convert your photos into a PDF document |
| **Compress / Resize Files** | Make large PDFs and images smaller, easier to share |

### 🔧 Powerful Tools
| Feature | What It Means |
|---|---|
| **Text Search in PDF** | Find any word inside a PDF instantly (Ctrl+F / Cmd+F) |
| **PDF Form Filling** | Fill out digital forms inside PDF files and save them |
| **Page Thumbnail Grid** | See all PDF pages at a glance and jump to any page |
| **Table of Contents** | Navigate large PDFs using chapter/section bookmarks |
| **Print** | Print any document directly from the app |
| **Night Mode** | Invert PDF colors for comfortable dark-room reading |

### 🖥️ Great Desktop Experience
| Feature | What It Means |
|---|---|
| **Drag & Drop** | Drag any file from your desktop or folder directly into the app |
| **3-Panel Layout** | Sidebar + Document Grid + Tools Panel — like a pro app |
| **Keyboard Shortcuts** | `Ctrl+F` to search, `Esc` to go back — fast and efficient |
| **Manage Scan Folders** | Choose which folders the app monitors for documents |

---

## 🛡️ Privacy — Your Files Stay Private

> **My Docs never uploads your files to the internet.**

- ✅ Works **100% offline**
- ✅ All processing happens **on your device**
- ✅ **No account** or login required
- ✅ **No ads** in this desktop version
- ✅ Your documents are **never shared** with anyone

---

## 🗂️ Supported File Types

| File Type | Extensions |
|---|---|
| PDF Documents | `.pdf` |
| Word Documents | `.doc`, `.docx`, `.rtf` |
| Excel Spreadsheets | `.xls`, `.xlsx` |
| Data Files | `.csv` |
| Images | `.jpg`, `.jpeg`, `.png`, `.webp`, `.heic` |

---

## ❓ Frequently Asked Questions

**Q: Is My Docs free?**
> Yes! My Docs is completely free to download and use.

**Q: Do I need an internet connection?**
> No. My Docs works 100% offline. Your files never leave your computer.

**Q: Can I open a file by double-clicking it?**
> Yes! On Windows and macOS, you can set My Docs as the default app for PDFs and documents. Then just double-click any file to open it.

**Q: Why does Windows/macOS show a security warning?**
> My Docs is not signed with an expensive developer certificate yet. The app is completely safe. Follow the steps in the [installation guide](#️-how-to-install) above to bypass this warning.

**Q: Will My Docs edit my original files?**
> Only the Excel Editor modifies files (when you explicitly save). The PDF viewer, Word viewer, and CSV viewer are **read-only** — your originals are never changed.

**Q: What should I do if the app doesn't open?**
> - **Windows**: Right-click the `.exe` → Run as Administrator.
> - **macOS**: Go to System Settings → Privacy & Security → Click "Open Anyway".
> - **Linux**: Make the file executable first (`chmod +x mydocs`).

---

## 💬 Need Help?

Having trouble? We're here to help!

[![WhatsApp Support](https://img.shields.io/badge/Chat_on-WhatsApp-25D366?style=for-the-badge&logo=whatsapp)](https://wa.me/919580552959?text=I%20need%20help%20with%20My%20Docs%20app)

Click the button above to send a WhatsApp message directly to the developer.

---

## 📋 System Requirements

| | Windows | macOS | Linux |
|---|---|---|---|
| **Minimum OS** | Windows 10 (64-bit) | macOS 10.15 Catalina+ | Ubuntu 20.04+ |
| **RAM** | 2 GB | 2 GB | 2 GB |
| **Storage** | 200 MB | 200 MB | 200 MB |
| **Architecture** | x64 | x64 / Apple Silicon | x64 |

---

## 🔄 Updates

New versions will be posted here in the **Releases** section. To update:
1. Download the latest version from the link above.
2. Install it the same way you installed the current version.
3. The new version will replace the old one automatically.

---

<div align="center">

**Made with ❤️ by Deependra Kashyap**

*My Docs — Manage your documents, your way.*

</div>
